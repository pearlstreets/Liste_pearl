              ...
              {(() => {
                ...
                return <>
                  <View style={{marginTop:8,flexDirection:"row",justifyContent:"space-between",alignItems:"center"}}>
                    <Text style={{fontSize:15,fontWeight:"600",color:"#374151"}}>{t('productsScreen.productsTotal')}</Text>
                  ...
                  <View style={{marginTop:4,flexDirection:"row",justifyContent:"space-between",alignItems:"center"}}>
                    <Text style={{fontSize:17,fontWeight:"700", color:"#111"}}>{t('cart.total') || 'Total'}</Text>
                    <Text style={{fontWeight:"800", fontSize:18, color:BRAND}}>{fmtPrice(liveTotal)}</Text>
                  </View>
                  {mode==="delivery" ? (
                    <View style={{marginTop:4,flexDirection:"row",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <View style={{backgroundColor:'#fff', borderTopLeftRadius:24, borderTopRightRadius:24, maxHeight:'70%', paddingBottom:40}}>
            ...
            <View style={{paddingHorizontal:16, gap:10}}>
              ...
              <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingVertical:8, borderTopWidth:1, borderTopColor:'#F3F4F6'}}>
                ...
                <Text style={{fontSize:13, fontWeight:'600', color:'#374151'}}>{popupSelectedItems.filter(si => checkedShops[si.shopIndex]).reduce((s, si) => s + (si.qty || 1), 0)}</Text>
              </View>
              <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:4}}>
                <Text style={{fontSize:16, fontWeight:'700', color:'#111'}}>{t('cart.total')}</Text>
                <Text style={{fontSize:18, fontWeight:'800', color:BRAND}}>{fmtPrice(popupSelectedItems.filter(si => checkedShops[si.shopIndex]).reduce((s, si) => s + (Number(si.price||0) * Number(si.qty||1)), 0))}</Text>
              </View>
              <TouchableOpacity
                onPress={async () => {
                  try {
