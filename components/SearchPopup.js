...
function enrich(item, priceFmt) {
  ...
  let subtitle =
    item.detail || item.subtitle || item.subTitle || item.description || item.info || '';
  if (!subtitle) subtitle = 'Origine France – 1kg';
  let unitPrice =
    item.unitPrice || item.pricePerKg || item.price_per_kg || item.secondaryPrice || '';
...
const SearchItem = React.memo(
  ({ item, selectedItems, onQtyChange, onQtyDelta, t, formatPrice, getEmoji }) => {
    const emoji = getEmoji(item.name);
    const translatedName = t('productNames.' + item.name, { defaultValue: item.name });
    const translatedDetail = item.detail
      ? t('productDetails.' + item.detail, { defaultValue: item.detail })
      : '';
    ...
    return (
      <View
        style={{
          backgroundColor: hasQty ? '#ECFDF5' : '#fff',
          borderRadius: 16,
          padding: 14,
          marginVertical: 8,
          shadowColor: '#000',
          shadowOpacity: 0.04,
          shadowRadius: 6,
          borderWidth: hasQty ? 1.5 : 0,
          borderColor: hasQty ? BRAND : 'transparent',
        }}
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'baseline', marginTop: 8 }}>
              ...
              {!!item.unitPrice && (
                <Text style={{ marginLeft: 8, color: '#9CA3AF' }}>{item.unitPrice}</Text>
              )}
            </View>
          </View>
          {qty === 0 ? (
...
export default function SearchPopup({
  ...
  initialQuery = '',
  data,
  shopNameFilter: _shopNameFilter,
  shopName,
  onClose,
  onSelect,
  ...
  // Memoize so the reference stays stable and doesn't invalidate the
  // renderItem useCallback on every render.
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.35)' }}>
          ...
          >
            ...
            <View style={{ padding: 16, paddingBottom: 0 }}>
              ...
              >
                <Text style={{ color: BRAND, fontWeight: '800', fontSize: 17 }}>
                  {shopName || t('searchPopup.title') || 'What do you need?'}
                </Text>
            ...
            <SafeAreaView style={{ backgroundColor: '#fff' }}>
              {selList.length > 0 && (
                <View style={{ borderTopWidth: 1, borderTopColor: '#E5E7EB' }}>
                  <ScrollView
                    style={{ paddingHorizontal: 16, maxHeight: 120 }}
                    ...
                    {selList.map((a) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text
                            style={{ flex: 1, fontSize: 13, fontWeight: '600', color: '#111' }}
                            numberOfLines={1}
                          >
                            {nm}
                          </Text>
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            {a.qty === 1 ? (
                              <RptBtn
                                onPress={() => handleRemove(a.item.id)}
                                onLongAction={() => handleRemove(a.item.id)}
                                style={{
                                  width: 24,
                                  height: 24,
                                  borderRadius: 12,
                              ...
                              <RptBtn
                                onPress={() => onQtyDelta(a.item, -1)}
                                onLongAction={() => onQtyDelta(a.item, -1)}
                                style={{
                                  width: 24,
                                  height: 24,
                                  borderRadius: 12,
                  ...
                  >
                    <Text style={{ fontSize: 14, fontWeight: '700', color: '#374151' }}>
                      {selList.reduce((s, a) => s + a.qty, 0)}{' '}
                      {t('productsScreen.quantity') || 'produits'}
                    </Text>
                    <Text style={{ fontSize: 15, fontWeight: '800', color: BRAND }}>
                      {formatPrice(selList.reduce((s, a) => s + a.item.price * a.qty, 0))}
                    </Text>
                  </View>
                </View>
              )}
              <View style={{ paddingHorizontal: 16, paddingVertical: 10 }}>
                ...
                >
                  ...
                  <Text style={{ color: '#fff', fontWeight: '800', fontSize: 16 }}>
                    {t('cart.addToCart') || 'Ajouter'} ({totalCount})
                  </Text>
                </TouchableOpacity>
              </View>
            </SafeAreaView>
