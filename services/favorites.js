...
export async function syncFavorites() {
  try {
    ...
    if (data.status) {
      ...
      // Sync products
      const products = (data.products || []).map(p => ({ name: p.name, detail: p.detail, price: p.price }));
  ...
  return { shops: shopsRaw ? JSON.parse(shopsRaw) : [], products: prodsRaw ? JSON.parse(prodsRaw) : [] };
}
